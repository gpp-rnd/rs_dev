...
def get_amino_acid_features(sg_designs, aa_seq_df, width=10):
    ...
    sg_aas['AA Subsequence'] = sg_aas.apply(lambda row: row['extended_seq'][row['seq_start'] - 1:row['seq_end']],
                                            axis=1)
    aa_features = featurize_aa_seqs(sg_aas['AA Subsequence'])
...
def build_target_feature_df(sg_designs, features=None,
    ...
    if 'aa' in features:
        aa_features = get_amino_acid_features(design_df, aa_seq_df, width=aa_width)
    ...
    if feature_df.isna().sum().sum() > 0:
        raise ValueError('Columns ' + ', '.join(['sgRNA Context Sequence', 'Transcript Base', 'AA Index']) +
                         ' have missing values')
    for key, df in feature_df_dict.items():
        feature_df = pd.merge(feature_df, df, how='left')
        if key == 'position':
            if feature_df.isna().sum().sum() > 0:
                raise ValueError('Position columns have missing value')
        else:  # domain, aa, or cons feature fill missing values with 0
